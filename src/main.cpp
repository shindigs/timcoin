...
uint256 hashGenesisBlock("0xf89aa670850a65e8974198852a767fd3b078fd5d3580df641fbaab7608c2d1e4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // timcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "timcoin Signed Message:\n";
...
{
    // timcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // timcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // timcoin: 1 days
static const int64 nTargetSpacing = 120; // timcoin: 20 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // timcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // timcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // timcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf89aa670850a65e8974198852a767fd3b078fd5d3580df641fbaab7608c2d1e4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        printf("%s\n + Merkle Root is me: ", block.hashMerkleRoot.ToString().c_str());
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // timcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("timcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static timcoinMiner(CWallet *pwallet)
{
    printf("timcoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running timcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("timcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&timcoinMiner, pwallet));
}
